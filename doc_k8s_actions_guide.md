...
> [!IMPORTANT] 工作负载资源（Deployment、StatefulSet或ReplicaSet等）在滚动升级时不会触发PDB。
...
> 才能在出现问题时不会手忙脚乱。  
> 此外，根据个人经验，如果你环境中的微服务数量低于10个，不建议使用Istio，因为它的上手和维护成本可能会超过其带来的收益。
...
这包含对HTTP系列协议和TCP协议的支持，本节不演示具体操作，请直接参考[官方文档][配置Istio网络拓扑]。
