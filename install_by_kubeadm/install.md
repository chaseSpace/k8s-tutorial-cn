...
# 2. 查看cert-hash
➜  practice openssl x509 -in /etc/kubernetes/pki/ca.crt -pubkey -noout |
...
token默认有效期24h。在过期后，我们需要手动创建一个临时token：

```shell
# cert-hash一般不会改变
➜  practice kubeadm token create --print-join-command                       
